...
class transcription:
    ...
    def __init__(self, stop_event, transcriptionApp):
        self.stop_event = stop_event
        self.transcriptionApp = transcriptionApp

    def transcribe(self, model_size='base', non_english=False, energy_threshold=500, record_timeout=4, phrase_timeout=3):

        Clock.schedule_once(lambda x: self.transcriptionApp.updateTranscriptionBox('Loading...'))
        ...
        #print("Listening...\n")
        Clock.schedule_once(lambda x: self.transcriptionApp.updateTranscriptionBox('Listening...'))
        ...
        while not self.stop_event.is_set():
            ...
            if not data_queue.empty():
                ...
                '''
                if _language == 'en':
                    Clock.schedule_once(lambda x: self.transcriptionApp.updateTranscriptionBox('\n'.join(transcription)))
                elif _language == 'ar':
                    transcription_modified = []
                    for line in transcription:
                        reshaped_text = arabic_reshaper.reshape(line)
                        bidi_text = get_display(reshaped_text)
                        transcription_modified.append(bidi_text)
        ...
        Clock.schedule_once(lambda x: self.transcriptionApp.updateTranscriptionBox('Stopped listening.'))
        #print('\nStopped listening.')